...
#### TY Jeff Pigott for sharing the script to change ARC Update Options
...
foreach ($server in $filteredServers) {
    ...
    Start-Job -ScriptBlock {
        New-AzConnectedMachineRunCommand -ResourceGroupName $using:server.ResourceGroupName -SourceScript $using:script -RunCommandName "runGetInfo50" -MachineName $using:server.Name -Location $using:server.Location
    }
}
